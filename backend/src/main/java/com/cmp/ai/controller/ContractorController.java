...
public class ContractorController {
    ...
    public ResponseEntity<byte[]> exportContractors(
            @RequestParam String month,
            @RequestParam(required = false) Long customerId,
            @RequestParam(required = false) String region,
            @RequestParam(required = false) String status,
            @RequestParam(defaultValue = "false") boolean includeFinancials) {
        byte[] fileBytes = contractorService.exportContractors(month, customerId, region, status, includeFinancials);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=contractors-export-" + month + ".xlsx")
